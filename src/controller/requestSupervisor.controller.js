...
const requestSupervisor = async (req, res, next) => {
	try {
		const body = req.body;
		const scholarData = (await Scholar.findById(req.user._id)).toObject();
		const supervisorData = (await Faculty.findById(body.supervisor_id)).toObject();
		...
		await request.save();
		await Scholar.findByIdAndUpdate(req.user._id, scholarData)
